  ...
  <div class="py-12">
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
      <div class="bg-white overflow-hidden p-10 shadow-sm sm:rounded-lg">
        ...
        <form method="POST" action="{{ route('admin.artikels.update', $artikel) }}" enctype="multipart/form-data">
          @csrf
          @method('PUT') {{-- Direktif untuk method update --}}
          {{-- Judul Artikel --}}
          <div>
            <x-input-label for="tittle" :value="__('Judul Artikel')" required />
            <x-text-input id="tittle" class="block mt-1 w-full" type="text" name="tittle" value="{{$artikel->tittle}}" required autofocus />
          </div>

          {{-- Slug Artikel --}}
          <div class="mt-4">
            <x-input-label for="slug" :value="__('Slug Artikel')" required />
            <x-text-input id="slug" class="block mt-1 w-full" type="text" name="slug" value="{{$artikel->slug}}" required />
          </div>
          {{-- Writer Artikel --}}
          <div class="mt-4">
            <x-input-label for="writer" :value="__('Penulis Artikel')" required />
            <x-text-input id="writer" class="block mt-1 w-full" type="text" name="writer" value="{{$artikel->writer}}" required />
          </div>
          ...
          <div class="mt-4">
            ...
            <img src="{{ Storage::url($artikel->thumbnail) }}" alt="Thumbnail Artikel" class="mb-4 rounded-md shadow-sm w-full h-auto">
