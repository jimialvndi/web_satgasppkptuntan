<x-app-layout>
  <x-slot name="header">
    <div class="flex flex-row justify-between items-center">
      ...
      <a href="{{ route('admin.artikels.create') }}" class="font-bold py-4 px-6 bg-indigo-700 text-white rounded-full">
        Add New
      </a>
    </div>
  </x-slot>
      ...
      <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
        ...
        <div class="item-card flex flex-col md:flex-row justify-between items-center border:1px solid #e5e7eb rounded-lg p-5 hover:shadow-lg transition-shadow duration-300">
          <div class="flex flex-row items-center gap-x-5 flex-1 min-w-0 mx-5">
            {{-- Thumbnail Image --}}
            <img src="{{Storage::url($artikel->thumbnail)}}" alt="Thumbnail" class="w-32 h-18 object-cover rounded-md">

            <div class="flex flex-col">
              <p class="text-slate-500 text-sm">Judul Artikel</p>
              {{-- Article Title --}}
              <h3 class="text-indigo-950 text-xl font-bold">{{$artikel->tittle}}</h3>
            </div>
          </div>

          {{-- Details section (Date & Writer) --}}
          <div class="hidden md:flex flex-row gap-x-10 text-left mx-5">
            <div class="flex flex-col">
              <p class="text-slate-500 text-sm">Created at</p>
              {{-- Format the date for better readability --}}
              <h3 class="text-indigo-950 text-l font-bold">{{$artikel->created_at->format('d M Y')}}</h3>
            </div>
            <div class="flex flex-col">
              <p class="text-slate-500 text-sm">Penulis</p>
              <h3 class="text-indigo-950 text-l font-bold">{{$artikel->writer}} </h3>
            </div>
          </div>
          ...
          <div class="flex flex-row items-center gap-x-3 mt-3 md:mt-0">
            <a href="{{ route('admin.artikels.edit', $artikel) }}" class="font-bold py-4 px-6 bg-indigo-700 text-white rounded-full">
              Edit
            </a>
        ...
        <div class="text-center text-gray-500">
          <p class="text-lg">Tidak ada Artikel yang tersedia.</p>
        </div>
        @endforelse
