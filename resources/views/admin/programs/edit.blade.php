      ...
      <div class="bg-white overflow-hidden p-10 shadow-sm sm:rounded-lg">
        ...
        <form method="POST" action="{{ route('admin.programs.update', $program) }}" enctype="multipart/form-data">
          ...
          <div>
            ...
            <x-text-input id="tittle" class="block mt-1 w-full" type="text" name="tittle" value="{{$program->tittle}}" required autofocus />
          </div>

          {{-- Tema Program --}}
          <div class="mt-4">
            <x-input-label for="theme" :value="__('Tema')" />
            <x-text-input id="theme" class="block mt-1 w-full" type="text" name="theme" value="{{$program->theme}}" required />
          </div>

          {{-- Deskripsi (Trix Editor) --}}
          <div class="mt-4">
            <x-input-label for="description" :value="__('Deskripsi')" required />
            <input id="description" type="hidden" name="description" value="{{$program->description}}">
          ...
          <div class="mt-4">
            ...
            <x-text-input id="date" class="block mt-1 w-full" type="datetime-local" name="date" value="{{$program->date}}" required />
          </div>

          {{-- Nama Tempat Lokasi --}}
          <div class="mt-4">
            <x-input-label for="location" :value="__('Lokasi')" required />
            <x-text-input id="location" class="block mt-1 w-full" type="text" name="location" value="{{$program->location}}" required />
          </div>

          {{-- Link Lokasi (Google Maps) --}}
          <div class="mt-4">
            <x-input-label for="location_link" :value="__('Link Lokasi (Google Maps)')" required />
            <x-text-input id="location_link" class="block mt-1 w-full" type="url" name="location_link" value="{{$program->location_link}}" required placeholder="https://maps.app.goo.gl/xxxx" />
          </div>

          {{-- Link Daftar (Google Form) --}}
          <div class="mt-4">
            <x-input-label for="regist_link" :value="__('Link Daftar (Google Form)')" />
            <x-text-input id="regist_link" class="block mt-1 w-full" type="url" name="regist_link" value="{{$program->regist_link}}" placeholder="https://forms.gle/xxxx" />
          </div>

          {{-- Upload Thumbnail --}}
          <div class="mt-4">
            <x-input-label for="thumbnail" :value="__('Upload Thumbnail')" />
            <img src="{{ Storage::url($program->thumbnail) }}" alt="Thumbnail Program" class="mb-4 rounded-md shadow-sm w-full h-auto">
